...
module.exports = function (config) {
  ...
  var TZOFFSET = (BASE_DATE_DEVICE - BASE_DATE_UTC);
  ...
  var getDeviceTime = function (t) {
    ...
    // Workaround: To get the time working for brandon we move the time 1 hour behind tzoffset.
    var ret = moment(new Date(time).getTime() + TZOFFSET - 3600000).toISOString().slice(0, -5);
    return ret;
  };
  ...
  var asanteBuildWizardRecords = function (data) {
    ...
    for (var i = 0; i < data.smartRecords.length; ++i) {
      ...
      wz.insulinCarbRatio = 0;
      wz.insulinSensitivity = 0;
      var refBolus = data.bolusIndexHash[wz.BolusID] || null;

      // setting carb recommendation to zero because it was wrong
      var rec = cfg.builder.makeWizard()
        .with_recommended({carb: 0, correction: refBolus ? (refBolus.normal || 0) : 0 })
  ...
  var asanteBuildBasalRecords = function (data) {
    ...
    for (var i = 0; i < data.basalRecords.length; ++i) {
      var rawBasal = data.basalRecords[i];
      /*
      ClicksDelivered: 9
      DateTime: 208613116
      Delivered_units: 0.45
      SeqNmbr: 26794
      UTCTime: "2014-08-11T20:05:12.194Z"
      bg: NaN
      type: 'basal'
      carbInput: undefined
      crc: 10539
      */
      ...
      var basal = {
        type: 'basal',
        scheduleName: 'Basal A',
        rate: rawBasal.Delivered_units,
        deliveryType: 'scheduled',
        time: getUTCTime(rawBasal.DateTime),
        deviceTime: getDeviceTime(rawBasal.DateTime),
        deviceId: 'Asante',
        source: 'Uploader',
        duration: 86400000
      };

      basal.timezoneOffset = cfg.timeutils.computeTimezoneOffset(basal.deviceTime, basal.time);

      if (prevBasal) {
        basal.previous = prevBasal;
      }

      prevBasal = _.cloneDeep(basal);
      delete prevBasal.previous;

      postrecords.push(basal);
    }

    postrecords[postrecords.length - 1].duration = null;
  ...
  return {
    ...
    uploadData: function (progress, data, cb) {
      ...
      var basals = asanteBuildBasalRecords(data);

      console.log('basals',basals);

      data.upload_records = data.upload_records.concat(basals);
